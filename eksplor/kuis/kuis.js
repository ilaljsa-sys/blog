(function() {
  ...
  let KOTOBA_DB = [];
  async function loadKotobaDB() {
    if (KOTOBA_DB.length > 50) return KOTOBA_DB;
    try {
  ...
  let BUNPOU_DB = [];
  async function loadBunpouDB() {
    if (BUNPOU_DB.length > 10) return BUNPOU_DB;
    try {
  ...
  function buatSoalKosakataDinamis(targetJml) {
    if (!KOTOBA_DB || KOTOBA_DB.length < 10) return [];
    ...
    for (let i = 0; i < targetJml; i++) {
      ...
      if (tipe === 'arti') {
        ...
        hasil.push({
          pertanyaan: `Apa arti dari kosakata「${displayWord}」dalam bahasa Indonesia?`,
          pilihan: pilihan,
          kunci: kunci,
          pembahasan: `Kosakata「${displayWord}」artinya adalah "${item.arti}".${item.contoh ? ` Contoh: ${item.contoh}` : ''}`
        });
      } else if (tipe === 'nihongo') {
        const formatOpt = (obj) => obj.kanji && obj.kanji !== obj.baca ? `${obj.kanji}[${obj.baca}]` : obj.baca;
        ...
        hasil.push({
          pertanyaan: `Bagaimana bahasa Jepang untuk kata「${item.arti}」?`,
          pilihan: pilihan,
          kunci: kunci,
        ...
        if (item.kanji && item.kanji !== item.baca) {
          const pilihan = [item.baca, pengecoh[0].baca, pengecoh[1].baca, pengecoh[2].baca];
          const kunci = Math.floor(Math.random() * 4);
          const temp = pilihan[0];
          pilihan[0] = pilihan[kunci];
          pilihan[kunci] = temp;
          ...
          hasil.push({
            pertanyaan: `Bagaimana cara membaca Kanji「${item.kanji}」yang tepat?`,
            pilihan: pilihan,
            kunci: kunci,
            pembahasan: `Kanji「${item.kanji}」dibaca sebagai「${item.baca}」yang artinya "${item.arti}".`
          });
        } else {
          const pilihan = [item.arti, pengecoh[0].arti, pengecoh[1].arti, pengecoh[2].arti];
          const kunci = Math.floor(Math.random() * 4);
          ...
          hasil.push({
            pertanyaan: `Apa arti dari kosakata「${displayWord}」?`,
            pilihan: pilihan,
            kunci: kunci,
            pembahasan: `Kosakata「${displayWord}」berarti "${item.arti}".`
          });
        }
      }
    }
  ...
  function buatSoalBunpouDinamis(targetJml) {
    if (!BUNPOU_DB || BUNPOU_DB.length < 5) return [];
    const pool = BUNPOU_DB;
    const poolLen = pool.length;
    const hasil = [];
    ...
    for (let i = 0; i < targetJml; i++) {
      ...
      if (i % 2 === 0) {
        ...
        hasil.push({
          pertanyaan: `Apa fungsi dan arti dari pola tata bahasa「${item.pola}」?`,
          pilihan: pilihan,
          kunci: kunci,
          pembahasan: `Pola「${item.pola}」berarti: ${item.arti}. Rumus: ${item.rumus}.${item.contoh ? ` Contoh: ${item.contoh}` : ''}`
        });
      } else {
        const pilihan = [item.pola, pengecoh[0].pola, pengecoh[1].pola, pengecoh[2].pola];
        const kunci = Math.floor(Math.random() * 4);
        const temp = pilihan[0];
        pilihan[0] = pilihan[kunci];
        pilihan[kunci] = temp;
        ...
        hasil.push({
          pertanyaan: `Manakah pola tata bahasa yang memiliki rumus:「${item.rumus}」?`,
          pilihan: pilihan,
          kunci: kunci,
          pembahasan: `Rumus「${item.rumus}」merupakan pola untuk「${item.pola}」yang berarti "${item.arti}".`
        });
      }
    }
    return hasil;
  ...
  function putarSuaraSoal(text) {
    if (!('speechSynthesis' in window)) return;
    window.speechSynthesis.cancel();
    const clean = String(text).replace(/\[.*?\]|\(.*?\)|(____)/g, ' ');
      ...
      <div class="space-y-7 animate-in fade-in duration-200">
        ...
        <div class="space-y-3">
          ...
          <p class="text-[11px] text-slate-400">
            Pilih opsi cepat atau <strong>ketik bebas</strong> jumlah soal yang Anda inginkan (1 sampai 1.000 soal maraton).
          </p>
        </div>
  ...
  window.mulaiSesiKuis = async function() {
    ...
    await Promise.all([loadKotobaDB(), loadBunpouDB()]);
    ...
    if (state.kategori === 'bunpou') {
      ...
      soalDihasilkan = dinamisKotoba.concat(cadangan);
    } else {
      const dinamisKotoba = buatSoalKosakataDinamis(state.jumlah);
      const dinamisBunpou = buatSoalBunpouDinamis(Math.ceil(state.jumlah / 2));
      soalDihasilkan = cadangan.concat(dinamisBunpou).concat(dinamisKotoba);
    }
      ...
      <div class="space-y-6 animate-in fade-in duration-150">
          ...
          <div class="flex items-center justify-between text-xs">
            <div class="flex items-center gap-2">
              ...
              <span class="text-slate-400 font-bold">
                Soal ${state.indeksSoal + 1} dari ${state.daftarSoal.length}
              </span>
            </div>
            ...
            <div class="flex items-center gap-2 sm:gap-3">
              ...
              <button 
                ...
                class="px-2.5 py-1 rounded-lg bg-sky-500/10 hover:bg-sky-500 text-sky-400 hover:text-white border border-sky-500/20 font-bold text-xs transition active:scale-95 flex items-center gap-1"
                title="Lewati soal ini jika sudah paham / terlalu mudah"
              >
                <span>Lewati</span>
              ...
              <button onclick="window.konfirmasiKeluarKuis()" class="text-slate-400 hover:text-rose-400 font-bold text-xs transition">
                ✕ Batal
              </button>
            </div>
          </div>
        ...
        <div class="p-6 sm:p-8 rounded-3xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-5 shadow-sm">
          <div class="flex items-start justify-between gap-3">
            ...
            <button onclick="window.putarAudioSoal('${(soal.pertanyaan || '').replace(/'/g, "\\'")}')" class="p-2.5 rounded-2xl bg-sky-500/10 text-sky-400 hover:bg-sky-500 hover:text-white transition shrink-0" title="Dengarkan Soal">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="pt-3 border-t border-slate-200/60 dark:border-slate-800/60 flex items-center justify-between">
            <span class="text-[11px] text-slate-400">Soal terlalu mudah atau sudah hafal?</span>
            <button 
              type="button" 
              onclick="window.skipSoalKuis()" 
              class="px-3.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-sky-500 hover:text-white text-slate-600 dark:text-slate-300 font-bold text-xs transition flex items-center gap-1.5 active:scale-95 shadow-sm"
              title="Langsung lewati ke soal berikutnya"
            >
              <span>Lewati Soal (Skip)</span>
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5l7 7-7 7M5 5l7 7-7 7"></path>
              </svg>
            </button>
        ...
        <div id="kuis-panel-pembahasan" class="hidden space-y-4 animate-in slide-in-from-bottom-3 duration-200">
          ...
          <div class="flex justify-end">
            <button onclick="window.lanjutSoalBerikutnya()" class="px-6 py-3.5 rounded-xl bg-sky-500 hover:bg-sky-400 active:scale-95 text-white font-black text-sm shadow-md transition flex items-center gap-2 group">
