...
const N3_API_URL = "https://script.google.com/macros/s/AKfycby7uH_OZjjEevBl18220gEdg6nQkBL9TPPd3lsB0uy4s1VVqqaDO30cuPjLBMQse82c2w/exec";
...
// Saklar Navigasi Mobile (Zen Mode)
...
// =========================================================================
// PARSER Kanji[furigana] -> <ruby>Kanji<rt>furigana</rt></ruby>
// =========================================================================
window.renderFuriganaHtml = function(text) {
  if (!text) return "";
  let parsed = String(text);

  // 1. Format Gemini Spreadsheet: Kanji[furigana] (mendukung kanji tunggal & jukugo)
  ...
  // 2. Format alternatif kurung biasa: Kanji(furigana)
  parsed = parsed.replace(/([一-龯々仝〆〇ヶ]+)\(([^\x00-\x7F]+?)\)/g, 
    '<ruby class="mx-0.5 font-bold">$1<rt class="text-[9px] text-sky-400 font-sans tracking-tight font-normal select-none leading-none">$2</rt></ruby>'
  );

  // 3. Format markdown: [Kanji](furigana)
  ...
  display.innerHTML = `
    <div class="w-full max-w-md lg:max-w-4xl mx-auto px-1 pb-16 transition-all">
      <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3 mb-6">
        <button onclick="window.tutupDisplayEksplor()" class="px-3.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-sky-500 hover:text-white text-slate-700 dark:text-slate-200 font-bold transition flex items-center gap-1.5 text-xs shadow-sm">
...
window.bukaHalamanMenuGoi = function() {
  ...
  display.innerHTML = `
    <div class="w-full max-w-md lg:max-w-3xl mx-auto px-1 pb-16">
      <div class="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3 mb-6">
        <button onclick="window.bukaModulN3()" class="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-sky-500 hover:text-white text-slate-700 dark:text-slate-200 font-bold transition flex items-center gap-1.5 text-xs shadow-sm">
...
// 3. FETCH DATABASE (SISTEM CACHE LOCALSTORAGE INSTAN & SET-SET)
window.ambilDataSheet = async function(sheetName, judul) {
  ...
  // Helper untuk memproses data mentah ke sesi aplikasi tanpa duplikasi kode
  const olahDataKeSesi = (dataArray) => {
    ...
    window.sesiGoiAktif.seluruhData = dataArray.map((item, idx) => ({
      serialNumber: idx + 1,
      uid: item.id || `${sheetName}_${idx + 1}`,
      minggu: Math.floor(idx / 22) + 1,
      kanji: item.kanji || item.pola || "-",
      baca: item.hiragana || item.rumus || "-",
      arti: item.arti || "-",
      contoh: item.contoh_kalimat || "",
      arti_contoh: item.arti_contoh || ""
    }));
  ...
  // 1. JALUR PINTAS INSTAN: Jika cache sudah tersimpan di browser, langsung buka UI
  if (dataCache) {
    try {
      ...
      if (parsed && Array.isArray(parsed) && parsed.length > 0) {
        ...
        // Lakukan sinkronisasi data baru di latar belakang tanpa mengganggu tampilan
        fetch(`${N3_API_URL}?sheet=${sheetName}`)
          .then(res => res.json())
          .then(hasil => {
            if (hasil.status === "success" && hasil.data && hasil.data.length > 0) {
              localStorage.setItem(cacheKey, JSON.stringify(hasil.data));
            }
          })
          .catch(() => {});
        return;
  ...
  // 2. JIKA BELUM PERNAH DIBUKA (Kunjungan pertama): Tampilkan pemuat
  display.innerHTML = `
  ...
  try {
    const res = await fetch(`${N3_API_URL}?sheet=${sheetName}`);
    const hasil = await res.json();

    if (hasil.status === "success" && hasil.data && hasil.data.length > 0) {
      // Simpan data array ke cache browser
      localStorage.setItem(cacheKey, JSON.stringify(hasil.data));
      olahDataKeSesi(hasil.data);
    } else {
...
window.bukaMenuPorsiHafalan = function() {
  ...
  display.innerHTML = `
    <div class="w-full max-w-md lg:max-w-2xl mx-auto px-1 pb-16">
      ...
      <!-- KOTAK MODE BUKU (BERSIH & SEJAJAR) -->
      <div class="mb-5">
...
window.renderModeBuku = function() {
  ...
  if (dataTerfilter.length === 0) {
    ...
    dataTerfilter.forEach(item => {
      ...
      if (isKanji) {
        ...
        // Pola Bunpou, Rumus & Contoh Kalimat Dirender Furigananya
        daftarHtml += `
          <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3 hover:border-sky-500/50 transition">
            ...
            <div class="p-3 rounded-xl bg-white/60 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-800">
  ...
  // Tombol Mata Minimalis Sejajar di Kanan Kolom Search
  ...
  display.innerHTML = `
    <div class="w-full max-w-md lg:max-w-4xl mx-auto px-1 pb-16">
      ...
      <!-- Live Search Box dan Tombol Mata Sejajar -->
...
// Toggle Flip Kartu Kanji di Mode Buku
...
window.renderLayarBelajar = function() {
  ...
  // KANJI FLASHCARD (KATA UTAMA TETAP GUNDUL: ${item.kanji})
  ...
  // KOSAKATA / KOTOBA (KATA UTAMA TETAP GUNDUL: ${item.kanji})
  display.innerHTML = `
    <div class="w-full max-w-md lg:max-w-2xl mx-auto px-1 pb-16">
      ${topHeader}
    ...
    <div id="n3-modal-contoh" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-lg p-6 space-y-4 shadow-2xl">
        ...
        <div class="space-y-3 py-2">
          <!-- Text Contoh Mengangkat Furigana ke Atas Tanpa Terpotong -->
...
// =========================================================================
// PREFETCH OTOMATIS: Ambil semua data diam-diam di latar belakang
// =========================================================================
window.addEventListener("DOMContentLoaded", () => {
  ...
  daftarSheetN3.forEach((sheetName, index) => {
    const cacheKey = `cache_n3_${sheetName}`;
    // Jika belum ada di memori browser, download secara bertahap tanpa mengganggu UI
      ...
      setTimeout(() => {
        fetch(`${N3_API_URL}?sheet=${sheetName}`)
          ...
          .then(hasil => {
            if (hasil.status === "success" && hasil.data && hasil.data.length > 0) {
              localStorage.setItem(cacheKey, JSON.stringify(hasil.data));
              console.log(`[Cache Ready] ${sheetName}`);
            }
          })
          .catch(() => {});
      }, index * 1200); // Dijeda 1,2 detik per sheet agar tidak menabrak batas rate-limit Google Apps Script
    }
  });
});
