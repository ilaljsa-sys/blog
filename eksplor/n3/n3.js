...
window.renderLayarBelajar = function() {
  ...
  const bottomNav = `
    <div class="flex items-center gap-2 pt-2">
      <button onclick="window.gantiKartu(-1)" title="Kartu Sebelumnya (Shortcut: ← Panah Kiri / A)" class="p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold transition text-xs shadow-sm">
        ◀
      </button>
      <button onclick="window.toggleTandaHafal('${item.uid}')" title="Tandai Hafal (Shortcut: M)" class="flex-1 py-3 rounded-2xl font-bold text-xs transition shadow-sm flex items-center justify-center gap-1.5 ${sudahDihafal ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 hover:bg-emerald-500 hover:text-white text-slate-700 dark:text-slate-300'}">
        ${SVG_ICONS.check}
        <span>${sudahDihafal ? 'Sudah Ditandai Hafal' : 'Tandai Hafal'}</span>
      </button>
      <button onclick="window.gantiKartu(1)" title="Kartu Berikutnya (Shortcut: → Panah Kanan / D)" class="p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold transition text-xs shadow-sm">
        ▶
      </button>
    </div>
  `;
...
window.addEventListener('keydown', function(e) {
  ...
  const modalContoh = document.getElementById("modal-contoh-n3");
  if (modalContoh) {
    if (e.key === 'Escape') {
      e.preventDefault();
      modalContoh.remove();
  ...
  if (!window.sesiGoiAktif || window.sesiGoiAktif.modeTampilan !== 'flashcard') return;
  ...
  if (key === ' ' || key === 'ArrowUp' || key === 'ArrowDown' || key === 'Enter') {
    e.preventDefault();
    window.toggleFlipCard();
    return;
  }

  if (key === '1') {
    e.preventDefault();
    if (typeof window.toggleSensorGoi === 'function') window.toggleSensorGoi('kanji');
    return;
  }
  if (key === '2') {
    e.preventDefault();
    if (typeof window.toggleSensorGoi === 'function') window.toggleSensorGoi('baca');
    return;
  }
  if (key === '3') {
    e.preventDefault();
    if (typeof window.toggleSensorGoi === 'function') window.toggleSensorGoi('arti');
    return;
  }
  ...
  if (key === 'm' || key === 'M') {
    ...
    const item = listTampil[window.sesiGoiAktif.indeksSekarang];
    if (item) {
      window.toggleTandaHafal(item.uid);
    }
    return;
  }
  ...
  if (key === 'p' || key === 'P') {
    ...
    if (item) {
      ...
      if (teksSuara) window.putarSuaraJepang(teksSuara);
    }
    return;
  }
