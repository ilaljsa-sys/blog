...
window.renderLayarBelajarN5 = function() {
  ...
  const bottomNav = `
    <div class="flex items-center gap-2 pt-2">
      <button onclick="window.gantiKartuN5(-1)" class="p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold transition text-xs shadow-sm">
        ◀
      </button>
      <button onclick="window.toggleTandaHafalN5('${item.uid}')" class="flex-1 py-3 rounded-2xl font-bold text-xs transition shadow-sm flex items-center justify-center gap-1.5 ${sudahDihafal ? 'bg-emerald-500 hover:bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 hover:bg-emerald-500 hover:text-white text-slate-700 dark:text-slate-300'}">
        ${SVG_ICONS_N5.check}
